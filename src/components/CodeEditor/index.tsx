...
export default function CodeEditor({ className, data, fileType }: Props) {
  const [copied, copy] = useCopyToClipboard(5000);
  const { addNotification } = useNotificationsContext();

  const handleClick = (text: string[] | undefined) => {
    if (text && !copied) {
      copy(text as unknown as string);
  ...
  return (
    ...
    >
      ...
      {data.map((item) => (
        ...
        >
          {item}
          {item.includes("NEXT_PUBLIC_APP_URL") ? (
            <>
...
function CopyIcon({ icon: Icon, onClick, colorClass, text }: CopyIconProps) {
  return (
